...
# Generate uImage with dtb for HDMI screen
cat ti-kernel/KERNEL/arch/arm/boot/zImage ti-kernel/KERNEL/arch/arm/boot/dts/am335x-boneblack-hdmi-overlay.dtb > zImage-dtb-hdmi
ti-kernel/KERNEL/scripts/mkuboot.sh -A arm -O linux -C none  -T kernel -a 0x80008000 -e 0x80008000 -n 'Linux-4.1' -d zImage-dtb-hdmi uImage-dtb-hdmi
if [ $? != 0 ]; then echo "ERROR"; exit; fi

# Generate uImage with dtb for eMMC/LCD cape
cat ti-kernel/KERNEL/arch/arm/boot/zImage ti-kernel/KERNEL/arch/arm/boot/dts/am335x-boneblack-emmc-overlay.dtb > zImage-dtb-emmc
ti-kernel/KERNEL/scripts/mkuboot.sh -A arm -O linux -C none  -T kernel -a 0x80008000 -e 0x80008000 -n 'Linux-4.1' -d zImage-dtb-emmc uImage-dtb-emmc
if [ $? != 0 ]; then echo "ERROR"; exit; fi

sudo cp uImage-dtb-hdmi /mnt/uImage
if [ $? != 0 ]; then echo "ERROR"; exit; fi
sudo umount /mnt
